...
import { Play, Globe, Home } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab, isPWA }) => {
  return (
        ...
        <div className="flex items-center gap-1 bg-white/5 rounded-full p-1 border border-white/10">
          {!isPWA && (
            <button 
              ...
              className={`px-3 md:px-5 py-1.5 md:py-2 rounded-full flex items-center gap-2 transition-all duration-500 ${activeTab === AppTab.HOME ? 'bg-white text-black shadow-lg scale-105' : 'text-white/60 hover:text-white hover:bg-white/10'}`}
            >
